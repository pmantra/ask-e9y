...
class OpenAILLMService(LLMService):
    ...
    def __init__(self, prompt_builder=None):
        ...
        self.prompt_builder = prompt_builder or PromptBuilder()  # Use provided or create new
    ...
    async def translate_to_sql(
        ...
        This method now uses PromptBuilder for prompt construction.
        """
        # Format schema for prompt
        schema_str = self._format_schema_for_prompt(schema_info)

        # Analyze the query
        analysis = self.prompt_builder.analyze_query(query)

        # Format examples if provided
        examples_str = self.prompt_builder.format_examples_str(similar_examples or [])
        ...
        # Build user prompt
        user_prompt = f"Generate a SQL query for: {query}"

        # Use translate_to_sql_with_prompt method
        return await self.translate_to_sql_with_prompt(
            query,
            system_prompt,
            user_prompt,
            conversation_id
    ...
    async def translate_to_sql_with_prompt(
        self,
        query: str,
        system_prompt: str,
        user_prompt: str,
        ...
        """Translate using provided prompts instead of generating them."""
        ...
        logger.debug(f"Generated system prompt with {len(system_prompt)} characters")
        logger.debug(f"Generated user prompt with {len(user_prompt)} characters")
        ...
        try:
            ...
            return sql, explanation, token_usage, prompts
...
"""}
    ...
    def _format_schema_for_prompt(self, schema_info: Dict[str, Any]) -> str:
        ...
        schema_str += "2. Find members by organization name: member.organization_id = (SELECT id FROM eligibility.organization WHERE name ILIKE '%Acme%')\n"
